...
def display_trajectory(trajectory):
    ...
    table = Table(
        title="Action Trajectory", show_header=True, header_style="bold green"
    )
    ...
    for idx, action in enumerate(trajectory, 1):
        details = (
            json.dumps(action.get("details", {}), indent=2)
            if isinstance(action, dict)
            else str(action)
        )
        action_type = (
            action.get("action", "Unknown") if isinstance(action, dict) else "Action"
        )
...
def create_status_entry(action, details):
    ...
    status_table.add_row(
        "[bold cyan]Status:[/bold cyan]", f"[cyan]{action.upper()}[/cyan]"
    )
    if details:
        status_table.add_row(
            "[bold yellow]Details:[/bold yellow]", f"[yellow]{details}[/yellow]"
        )

    return Panel(status_table, border_style="green", padding=(0, 1))
...
def get_task_configuration():
    ...
    if use_schema:
        ...
        try:
            ...
            console.print(
                "[yellow]Invalid JSON schema. Proceeding without schema.[/yellow]"
            )
            schema = None

    return {"task": task, "headless": headless, "schema": schema}


async def main():
    parser = argparse.ArgumentParser(
        description="Web Agent Console Interface - Browser automation with AI assistance"
    )
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            # Display duration
            console.print(
                f"[cyan]Duration:[/cyan] {result.get('duration_seconds', 0):.2f} seconds"
            )
        else:
            console.print(
                Panel(f"[green]Result:[/green]\n{result}", border_style="green")
            )
